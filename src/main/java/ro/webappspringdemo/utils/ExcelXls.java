...
public class ExcelXls {
     ...
     * @throws IOException
     */
    public ExcelXls(InputStream file) throws IOException {
    ...
    public ExcelXls(InputStream file, boolean checkForDuplicates) throws IOException {
        mapper = new ObjectMapper();
        wb = new HSSFWorkbook(file);
        totalSheetNr = wb.getNumberOfSheets();
        encountered = new HashSet<>();
    ...
    public List getDataFromXls(Class<?> objectType) {
    ...
    public List getDataFromXls(Class<?> objectType, int extractionOption) {
        setHeader(objectType);
        List objects = new ArrayList<>();
